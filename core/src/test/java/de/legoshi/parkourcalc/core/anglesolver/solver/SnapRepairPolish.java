...
public final class SnapRepairPolish {
    ...
    private static final class Search {
        ...
        void updateTransBest(float[] gf, ForwardPath path) {
            ...
            PathTranslation.Trans tr = PathTranslation.bestTranslationObj(compiled, gfD, path, loX, hiX, loZ, hiZ,
                    objAxisX ? 0 : 1, objMax);
                ...
                for (int dv = 0; dv <= MAX_NUDGE_ULPS; dv++) {
                    ...
                    for (int sx = -1; sx <= 1; sx += 2) {
                        for (int sz = -1; sz <= 1; sz += 2) {
                            double tx = PathTranslation.clampAxis(PathTranslation.ulpShift(tx0, sx * du), loX, hiX);
                            double tz = PathTranslation.clampAxis(PathTranslation.ulpShift(tz0, sz * dv), loZ, hiZ);
