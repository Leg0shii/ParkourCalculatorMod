...
public final class StratMeasure {
    ...
    public static final float KEY_INPUT_SCALE = StratVariants.KEY_INPUT_SCALE;
    public static final float SNEAK_INPUT_SCALE = StratVariants.SNEAK_INPUT_SCALE;

    private static final Gson GSON = new Gson();

    private StratMeasure() {
    }

    public static final class BadSaveException extends RuntimeException {
        public BadSaveException(String message) {
            super(message);
        }
    }

    public static StratMeasurements measure(SaveFile file, String name) {
        ...
        if (recorded != null && !recorded.success) {
            throw new BadSaveException(name + ": recorded result is marked success=false (met "
            ...
            for (SaveFile.Tick tick : file.angleSolver.ticks) {
                ...
                for (SaveFile.Constraint c : tick.constraints) {
                    if (c != null && !c.disabled
                        ...
                        throw new BadSaveException(name + ": enabled constraint at tick " + tick.tick
        ...
        StratMeasurements m = new StratMeasurements();
        m.name = name;
    ...
    public static SaveFile withSmoothedLine(SaveFile file, String name) {
        ExactJumpModel model = ExactJumpModel.forMcVersion(file.mcVersion);
        JumpSpec spec = buildSpec(file, model);
        if (spec == null) {
            return null;
        ...
        double[] yawVec;
        try {
            yawVec = recordedYaws(file, name, sc.numTicks);
        } catch (RuntimeException ex) {
            return null;
        }
        if (violation(model, sc, compiled, yawVec) > 0.0) {
            return null;
        }
        double[] anchor = centeredAnchor(model, sc, oracle, yawVec);
        double[] smoothA;
        double[] smoothR;
        try {
            smoothA = smoothestLine(model, sc, oracle, anchor, name);
            smoothR = smoothestLine(model, sc, oracle, yawVec, name);
        } catch (RuntimeException ex) {
            return null;
        }
        double[] extA = withStart(sc.startYaw, smoothA);
        double[] extR = withStart(sc.startYaw, smoothR);
        double[] smooth = accelSq(extR) <= accelSq(extA) ? smoothR : smoothA;
        double[] rounded = new double[smooth.length];
        for (int k = 0; k < smooth.length; k++) {
            rounded[k] = (float) smooth[k];
        }
        if (violation(model, sc, compiled, rounded) > 0.0) {
            return null;
        }
        SaveFile copy = GSON.fromJson(GSON.toJson(file), SaveFile.class);
        int startTick = copy.angleSolver.startTick;
        for (int k = 0; k < rounded.length; k++) {
            int r = startTick + k;
            if (r < 0 || r >= copy.rows.size()) {
                return null;
            }
            copy.rows.get(r).yaw = (float) rounded[k];
            copy.rows.get(r).yawLocked = true;
        }
        if (copy.angleSolver.result != null && copy.angleSolver.result.yaws != null) {
            for (SaveFile.Yaw y : copy.angleSolver.result.yaws) {
                int k = y.tick - startTick - 1;
                if (k >= 0 && k < rounded.length) {
                    y.yaw = rounded[k];
                }
            }
        }
    ...
    public static ForwardPath linePath(SaveFile file, String name) {
        ExactJumpModel model = ExactJumpModel.forMcVersion(file.mcVersion);
        JumpSpec spec = buildSpec(file, model);
        if (spec == null) {
            return null;
        ...
        JumpPhysicsInputs sc = spec.asScenario();
        double[] yaws = recordedYaws(file, name, sc.numTicks);
        double[] gf = sc.toGameFacings(Angles.wrapAll(yaws));
        return model.forward(sc, gf);
    }

    public static SaveFile shiftedCopy(SaveFile solved, int edgeRow, int shift) {
        ExactJumpModel model;
        JumpSpec spec;
        try {
            model = ExactJumpModel.forMcVersion(solved.mcVersion);
            spec = buildSpec(solved, model);
        } catch (RuntimeException ex) {
            return null;
        }
        if (spec == null) {
        ...
        double[] yawVec;
        try {
            yawVec = recordedYaws(solved, "shift", sc0.numTicks);
        } catch (RuntimeException ex) {
            return null;
        }
        return shiftedProbe(GSON.toJson(solved), model, sc0, yawVec, edgeRow, shift,
                solved.angleSolver.startTick);
    }

    public static double[] smoothestLine(ExactJumpModel model, JumpPhysicsInputs sc,
    ...
    public static JumpConstraintCompiler.Compiled landingOracle(JumpConstraintCompiler.Compiled compiled) {
        List<JumpConstraint> ineq = new ArrayList<JumpConstraint>();
        List<JumpConstraint> eq = new ArrayList<JumpConstraint>();
    ...
    private static void countInputEdges(SaveFile file, StratMeasurements m, int takeoffRow, int lastRow) {
    ...
    private static void measureShiftWindows(SaveFile file, String name, ExactJumpModel model,
                                            JumpPhysicsInputs sc0, StratMeasurements m, double[] yawVec) {
        int n = sc0.numTicks;
    ...
    public static double effWidth(StratMeasurements m, int i) {
    ...
    private static double shiftGeoEff(StratMeasurements m, boolean momentum) {
        double sum = 0.0;
        int count = 0;
    ...
    private static double shiftMin(StratMeasurements m, boolean momentum) {
    ...
    private static double shiftGeo(StratMeasurements m, boolean momentum) {
        double sum = 0.0;
        int count = 0;
        for (int i = 0; i < m.shiftEdgeRow.length; i++) {
            if (m.shiftEdgeMomentum[i] != momentum) {
    ...
    private static boolean probeShift(String baseJson, ExactJumpModel model, JumpPhysicsInputs sc0,
        ...
        return shiftedProbe(baseJson, model, sc0, yawVec, edgeRow, shift, startTick) != null;
    }

    private static SaveFile shiftedProbe(String baseJson, ExactJumpModel model, JumpPhysicsInputs sc0,
                                         double[] yawVec, int edgeRow, int shift, int startTick) {
        ...
        if (!supported(path, support)) {
            return null;
        }
        return copy;
    }

    public static boolean applyShift(SaveFile copy, int t, int shift, int startTick, JumpPhysicsInputs sc0,
    ...
    public static void syncMovementSample(SaveFile copy, int row, int srcRow) {
        syncMovementSample(copy, row, srcRow, true);
    }

    public static void syncMovementSample(SaveFile copy, int row, int srcRow, boolean syncSprint) {
    ...
    public static boolean supported(ForwardPath path, List<double[]> support) {
    ...
    public static boolean groundedAt(JumpPhysicsInputs sc, int k) {
    ...
    public static double violation(ExactJumpModel model, JumpPhysicsInputs sc,
    ...
    public static double[] recordedYaws(SaveFile file, String name, int n) {
        if (file.angleSolver.result == null || file.angleSolver.result.yaws == null
                || file.angleSolver.result.yaws.isEmpty()) {
            throw new BadSaveException(name + ": capture carries no result.yaws, re-solve and re-save");
        }
        int startTick = file.angleSolver.startTick;
    ...
    public static JumpSpec buildSpec(SaveFile file, ExactJumpModel model) {
