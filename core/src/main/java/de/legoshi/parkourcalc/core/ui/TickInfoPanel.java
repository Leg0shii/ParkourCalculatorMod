...
public final class TickInfoPanel implements RenderInterface {
    ...
    private void renderTable(int idx, TickState cur, TickState prev, TickState prev2) {
        ...
        if (prev != null && prev2 != null) {
            ...
            accentRow("Acceleration (XZ)", String.format(Locale.US, FMT_PAIR, dx - pdx, dz - pdz),
                    "Per-axis change in post motion: (deltaX(i) - deltaX(i-1), deltaZ(i) - deltaZ(i-1)).");
        } else {
            accentRow("Acceleration (XZ)", NA,
                    "Per-axis change in post motion: (deltaX(i) - deltaX(i-1), deltaZ(i) - deltaZ(i-1)).");
        }
    ...
    private void accentRow(String label, String value, String tooltip) {
        ImGui.tableNextRow();
        ImGui.tableNextColumn();
        float[] c = settings.tickWall;
