...
Status (updated 2026-07-05): **the convex floor is now solid.** The `solveJoint` bug diagnosed in Section 9 is fixed, the engine calls it, and the `:core:test` gate is green (328 tests, anchored path byte-identical). The remaining ambition it named then (a rebuilt nonlinear CMA search, escalation, parallelism) was overtaken: CMA-ES was removed entirely (PRs 371/373/375) and the nonlinear side became the dual/joint machine of gh-283 plus the seed-independence work (PRs 387-390). See Section 0 (top) for the resolution.

Original status (pre-fix): **works with caveats, but has bugs; the free-start SEARCH is not trustworthy yet.** Phases 0-2 of `docs/research/free-start-position-plan.md` (consolidated into Section 10 below and deleted, 2026-08-21) are implemented and the `:core:test` gate is green, but the in-app behaviour on real jumps is wrong (an *easy* jump returns 6/7 = fails to land). This document is the ground truth for the next session. Read `free-start-position-plan.md` first for the original design, then this. (Annotation 2026-08-21: that plan file is deleted; its design rationale is carried in Section 10.)
...
**Engine wiring (updated 2026-08-21):** the current entry point is the `FreeStartImproveNode` graph node calling `FreeStartSolve.solveJointBest`: the smoothed attempt first, then warm rungs at 2e-3 and 5e-4, then the center-started seed lane, then the certify tail. The CMA fallback originally described here was removed (PRs 371/373/375); seed independence for dF=0 chains shipped in commit e171cc52 (PRs 387-390).

**Files:** `CostateDualSolver.java` (smoothing), `FreeStartSolve.java` `solveJoint` (centre reference), `AngleSolverEngine.java` `freeStartImprove` (wiring), `FreeStartSolveTest.java` + `ThoroughAdoptFeasibilityTest.java` (regression). This shipped in the v1.7.0 position-free-solver work (dc0fe939) and later evolved; see the engine-wiring note above for the current shape. An adversarial review flagged one candidate (THOROUGH adopting a seed-feasible-but-adopted-infeasible candidate); it was refuted both by object identity (`sc == spec.asScenario()`, mutated in place) and empirically (the sub-solvers return adopted-start-feasible results on the exact trigger). **Unverified in-game** (the apply/start-move sim retrigger, per Section 7, still needs a real client run).
...
**Sections 3 to 9 below are the historical CMA-era record; that machinery is deleted. Kept for the failed-approach catalog and the section 9 f2f diagnosis.**
...
## 10. Design rationale (carried 2026-08-21 from free-start-position-plan.md, since deleted)

The original plan document is deleted; this section carries its design rationale so the handoff stands alone.

**Exactness (plan §2).** `JumpLinearModel` writes horizontal position as `pos_k = p0_k + Σ_{s<k} C(s,k)·u_s` with `p0_k = startPos + initialVelocity·sPre[k]`. Two facts: (1) `startPos` enters every tick as a pure additive constant with coefficient 1 (horizontal MC movement reads no absolute X/Z), so shifting it by δ shifts every `pos_k` by δ, shape unchanged; translation invariance, a rigid translation of the whole path. (2) `initialVelocity` enters linearly too but with weight `sPre[k]` that grows with the horizon: a shear that tilts the later path more than the start. They are not interchangeable (moving the start and changing the momentum produce different landing patterns), and both can be lifted. Because both enter every position constraint linearly, freeing them as box-bounded variables keeps the problem convex; the only nonconvexity stays the per-tick yaw modulus. Where the constant was baked in: `compileWall` folds `constVal` (which includes `p0` via `posAt`) into each wall's `bPrime`, and `objectiveVectors` does the same for the objective; lifting a variable means moving its contribution from that constant RHS into a decision column. This is why freeing the start preserves the model's exactness.

**Free vs anchored is box width (plan §3).** A start is only free when it is a genuine choice (the player stands on a block and picks a spot); an anchored start (a mid-route jump, or a receding-horizon window seeded from a committed jump) is *produced by the prior tick*, an output, and must never move. One mechanism unifies both: anchored solves get a zero-width box and reduce byte-for-byte to the fixed-start solve; freeing the start is a non-degenerate box, never a separate code path. This is what preserves causality: a seam can never be teleported because its box has zero width by construction.

**Multi-jump (plan §3).** Only jump 1 has a free start; jumps 2..N start from the previous jump's exact landing (dependent, never free; `LongRunSolver` chains byte-exact exit state forward as the next window's fixed seed, and that must stay). Shifting jump 1's start rigidly translates the entire downstream route (every jump is translation-invariant, so the whole chain shifts together, each seam still equal to its predecessor's output), so a route has exactly one free translation, pinned by whichever constraint binds anywhere along the route. Validity caveat as the plan stated it: a rigid translation is only exact while it does not change the tick structure (landing one tick earlier or later, or on a different block, changes the ground/air masks); within a one-block footprint the structure is stable, so it holds as a local convex model, and the byte-exact `SimulatorEntity` verify re-certifies; if a footprint edge would flip the structure, that edge becomes the binding constraint.

**Realizations (plan §5).** (A) Carry the free start inside `CostateDualSolver` as bounded deviation variables: they enter the Lagrangian linearly and each contributes its box support function to the dual, giving jointly optimal `(start, yaws)` in one convex solve. This is what shipped as `FreeP0` (Section 2, fixed per Section 0). (B) Outer translation: keep the dual untouched, solve the relative problem, then a small 2D (or 4D) LP for the translation over the box against the compiled constraint slacks, re-solving yaws if the active set changed. This survives as `bestTranslate`/`pinTranslate`.

**Trigger.** The plan's decision record (free the start iff `startTick == 0` AND the first tick carries an X and/or Z IN-range constraint; the range IS the footprint; no block capture) is already stated in full in Section 1 above; not duplicated here.
