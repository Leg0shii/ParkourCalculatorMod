...
- Going lower does NOT keep working: at 1e-4, 1e-5, and 1e-6 the nosolve save FAILS again (the 1/P0_SMOOTH start-term curvature stiffens the Hessian until the joint solve degrades). The working range for this pair is a window, roughly [5e-4, 2e-3]. The smoothing cannot be "removed"; the solver needs it for conditioning, and every single value is tuned luck for some jump. The anti-lottery structure is a LADDER on the failure path (0.05 then 0.002 then 5e-4, warm-started, exact translation plus byte-exact certify per rung, first certify wins): union semantics can only add solves, never remove one, and captures that solve at the smooth rung never see the sharp ones.
...
**Seed-lane kill-audit (issue #386 follow-up, same session):** counters at the three seed-lane adoption points over the sweep and the frac screen: the two FreeStartImproveNode entries are dead (0 wins in 4 calls); solveJointBest's entry is load-bearing (6 sweep wins, 16 frac wins). Disabling the lane entirely keeps EVERY outcome (sweep 104/104, all frac corners unchanged), so it is outcome-redundant, but the cost is prohibitive: 29 -> 81 s sweep, j347 base 4.2 -> 24.1 s (deep toward the 45 s budget); the lane is a fast path, not a capability. Resolution: instead of deleting it, its one seed-dependent input (the iteration start point, `box.px` = the clamped seed) now derives from the box CENTER, making the whole rescue machinery constraint-derived. The two FreeStartImproveNode `solve()` fallbacks (the 0-win entries) were then removed: they are dead by construction, not just by measurement, because `solveJointBest` runs the identical `solve()` internally before returning, so a node-level retry re-executes a deterministic computation that just returned null (and burned a redundant lane pass doing it). Measured: 104/104, zero outcome changes, zero captures meaningfully slower, sweep at baseline. Remaining seed influence after this: only the engine's primary pinned-window solve, which can return a solution reachable from a directly-feasible standing position (it can no longer flip solve/no-solve); demoting that is a UX ruling, not a bug fix.

**j335 low-X diagnosis (issue #386 follow-up, same session):** step-level logging through the joint ladder found three stacked facts. (1) The margin bisection's feasibility oracle is unsound under the iteration cap: on j335's 0.0625-wide Z corridor, margins above the 0.03125 half-width are infeasible, but the unbounded duals diverge slowly (lambda ~300-600 on all 27 walls, dual values -606, capped at 100 iterations far below LAMBDA_CAP), so tStar inflated 5.5x to 0.171 and the recovery rungs solved at infeasible margins, producing corridor-violating garbage. Fixed exactly and cheaply: an analytic opposing-pair margin cap (for antipodal wall pairs, margin <= (b+ + b-)/2), which lands tStar at 0.03125 (free pattern) and 0.005 (inertia-wall pairs). A weak-duality dual-value floor was also tried and is sound but subsumed; not kept. (2) At honest margins the recovery still floors at 4.3e-2..0.3: the corridor's 11 opposing pairs form a flat lambda-ridge (costate contributions cancel), the classic degenerate-recovery landscape; no amount of iteration or smoothing changes it. (3) The cure for that class already exists and was measured: the full pinned dual chain (closed form -> relaxation recovery) solves j335 AT THE BOX CENTER in 186 ms with violation 0 under the free spec; the base variant only ever solved because its seed IS the center. The free-rescue node now runs the pinned chain at the derived center when the joint machine fails; j335 solves from all 5 screened seeds (the former 37 s timeouts land in ~1.6-2.4 s), the corpus-wide in-box parity screen is fully clean, and the class is pinned as problems/solve/gh386-j335-lowx. Cost: the added chain runs on rescue-failure paths; j346 pays most (base 2.2 -> 5.5 s), sweep total unchanged (~28.5 s) because j335's wins offset it.
