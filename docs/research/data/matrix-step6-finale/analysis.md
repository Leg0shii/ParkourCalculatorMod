...
## Postscript 2: the smoothness metric arc (same day, in-game iteration)

Three metrics were tried against the user's eye on real jumps; only the third survived. Travel
(L1 yaw distance) let micro-wiggles ride free (they are nearly travel-neutral and each buys
objective). Travel plus a per-reversal cost inverted into freeze-and-snap staircases at strong
lambda (minimum-travel paths concentrate all turning into single ticks; the metric scores a
staircase as perfectly smooth). The shipped metric is JERK: the L1 second difference of yaw
(Angles.wiggleDeg). Constant-rate sweeps of any size score zero; staircases, zigzags, and
visible micro-jitter all score as what they look like. Lesson recorded: travel-family
smoothness metrics are degenerate for human-perceived smoothness; the eye keys on turn-rate
continuity.

Mechanism shipped with it: SmoothingPolish's acceptance gate relaxes from the raw-objective
floor to the scored floor when lambda > 0 (ratcheting, lambda=0 byte-identical). This is the
entire reason the toggle works on Fast: stopOnFeasible exits before any search stage, so the
smoothing pass is the only smoothness lever Fast has, and its old floor refused every move.

Pin measured on the user-corrected benchmark set (j001 + taser-80t + taser-100t only; j002/j003
are same-jump duplicates and nix-class verifier jumps are smoothness-irrelevant, per user
ruling): jerk plateaus at lambda 1e-2 (taser-80t 117 -> 73.5, Fast reaches the same plateau in
0.1 s), 1e-1 adds nothing for extra objective cost, so TASER_SMOOTH_LAMBDA = 1e-2. At the
plateau, taser-100t returns the entire +1.24-block CMA gain: that gain was jitter-bought, and
smooth mode's contract (smoothness first, objective as tiebreak) correctly refuses it.

353t ceiling: smoothing DOES run above the 256 cap (smoothFinal is not tick-gated), reduces
j001 jerk ~15% at 1e-2, and converges there structurally; scaling the eval budget with tick
count (evalsPerTick=400) bought only a further 1.5%. The residual jag is generated by the
receding-horizon windows themselves; making those lambda-aware collides with the feasibility
firewall and needs a design session, not a patch.

OPEN AND BLOCKING before further smoothness numbers are trusted: smoothPenalty call sites feed
mixed yaw arrays (some game facings with realized 45-flicks, some base solver yaws), and the
recorded jerk stat may not be the gate's jerk (the Fast taser-100t sweep rows show objective and
recorded jerk rising together, which a clean ratchet forbids). Unify on base yaws: smooth mode
must never tax the 45-strafe technique itself, which is the fastest movement in the game.
Replay-camera smoothing of the 45 pattern was explored and REJECTED by the user; the replay
path stays exact and untouched.
