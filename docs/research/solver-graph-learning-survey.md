...
- **L1, static portfolio + cheap pre-solve. [verified pattern]** Hydra-style greedy marginal-contribution selection over recorded runs on `problems/solve` + `problems/closedform` to pick 3-5 complementary presets; keep a cheap first-try stage (closed-form + short CMA; the CMA half is historical, CMA-ES was removed 2026-08) in front, SATzilla's 32-62%-in-7s lesson. Racing the portfolio with timeouts is already a strong baseline.
...
Since shipped: `SolveRunRecord` and `SolveRunLog` live in `core/.../anglesolver/graph/`, wired in `Application` to `<saveDir>/runs/`; the matrix machinery (`RunMatrixScreen`, `MatrixAnalysisScreen`, `CaptureMutations`) lives in the core test tree.
...
Setup: `RunMatrixScreen` (core test sources), every problem under `problems/solve` (29) and `problems/closedform` (15) x 5 presets, cold starts, sequential, 120 s cap per run (cap = censored CANCELLED record), one `SolveRunRecord` JSONL line per run. 220 runs, 1 h 41 min wall. Artifacts: `docs/research/data/matrix-full1/` (`analysis.md` is the durable record; the raw `runs.jsonl` was dropped in the 2026-08 doc cleanup; regenerate analyses with `MatrixAnalysisScreen`). 15 captures are wired to both checks, so the 44 problem rows cover 29 unique problem hashes; duplicated rows agree, raw numbers below.
...
CMA-ES and momentum assembly were removed in the Aug 2026 simplification train (PRs 371-379), so the preset mechanisms named in sections 8-8c (the CMA race, cma-only and bnb-heavy variants as built, momentum assembly) are historical. `nix-full-t1` is now an accepted-fail expect: the momentum-assembly pathway that solved it in 8c no longer exists. loopmm solves via the coldBnb stage at THOROUGH. The NO-GO verdict on the learned selector and the capability-frontier/band ladder stand.
