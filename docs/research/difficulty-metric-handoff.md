> **STATUS 2026-08-11: this is combinedV4, the MEASUREMENT-LAB difficulty metric - NOT the cold-tool placeholder. Canonical: `stratfinder-coldsearch.md` (section 9).** Do NOT read the consolidated doc's "difficulty was a placeholder, now real" as referring to combinedV4 - that referred to the COLD strat-finder's separate difficulty (facing-band width primary). combinedV4 (0.960 Spearman, smoothest-feasible-line jerk) is mature and stands. STILL VALID and important as traps: (1) the simplify-loop VERDICT 2026-07-28 (outputs withdrawn - the loop edited keys while saves were in Force-45 mode where the model ignores keys; "cold-verified" only meant the headless model agreed with itself, the real SimulatorEntity was never in the loop - this is the "simulator verify gate" lesson); (2) SPRINT-key edges are NOT probeable by key mutation (sprint is stateful; probe the recorded sprint FLAG chronology); (3) holdJump is NOT physics-identical for fires <10 ticks apart (10-tick jumpTicks cooldown the model omits). STALE: j154 "skipped as a defective save (success=false)" - j154 is now the finale capture with a stored byte-exact line (cold docs); j030 re-save may still be outstanding.
...
- Trigger: Forge 1.8.9 only for now (all 34 captures are mcVersion 1.8.9): keybind J (`key.parkourcalculator.run_sim_verify`, moved from V after dev's solver hotkeys took V), registered ONLY when the `PKC_SIMVERIFY` env var is set (it confused normal play: on a world without the staged jumps every save fails, which is correct but alarming). The env value is the save directory; `1` or empty means the default `<mcdata>/parkourcalculator/simverify`. Summary goes to log + chat.
