...
"Read docs/research/split-solver-grill-brief-2026-07-19.md sections 3, 4, 6 and 10 (steps 1-4 done: A22 = budget semantics; A8 = wiggle search-intrinsic; staged late-race shipped and gated, loopmm-tight-t39 solves under FAST in ~36 s). FIRST: in-game verify of the race (solve a loopmm-class jump under Fast, watch the explore samples land in the incumbent strip) plus the M3-M5 editor pending verifies. THEN execute checklist step 5: the objective abstraction (run-level scoring config {base + lambda*wiggle} through scoredObjective, SolveRunRecord.metric records lambda, ladder stays lambda=0, create the TASer band, pin lambda from the matrix-a8base1 scale hints in that analysis.md)."
