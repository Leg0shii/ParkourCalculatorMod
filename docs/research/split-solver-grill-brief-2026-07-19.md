...
"Read docs/research/split-solver-grill-brief-2026-07-19.md sections 3, 4, 6 and 10 (steps 1-5 done: staged late-race in-game verified 2026-07-20; lambda abstraction shipped, pin = 1e-3, TASer band at docs/research/data/matrix-taser-pin1/). Execute checklist step 6: the bake-off (incumbent chain at taser60-l1e3 vs AlmSnapStage-with-native-lambda) on the TASer band, and build the param-sweep mode (A18) it needs. Three prerequisites the step-5 data forces: (1) the band has NO 80/100-tick rungs (corpus gap between 66 and 176 ticks); produce them, e.g. by slicing the j001/j002/j003 captures, before the bake-off can run at its specified lengths; (2) the incumbent contender is lambda-INERT over the 64-tick router cap (receding-horizon/SLP windows never consult lambda), so decide whether the bake-off tests it as-is (ALM presumably wins at length by default), extends SLP with a travel term, or rethinks the tick cap; (3) AlmSnapStage needs lambda added natively to its smooth objective (A7 said 'natively in ALM'); it has no lambda today."
