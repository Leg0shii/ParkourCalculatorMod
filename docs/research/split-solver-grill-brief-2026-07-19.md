...
- A5. Split evidence is n=2. VERDICT: firmed 2026-07-19 (matrix-race-gen1). The flip pair behaves exactly as predicted under the race (loopmm 36 s via explore arm, duplicated base identical; nix 64-70 s via primary); the gen-ladder mutants are UNCHANGED (all-fail rungs still fail at cap, easy g-2 rungs still instant), consistent with the survey ruling that those rungs are capability frontier, not config, and confirming the race regresses nothing.
...
"Read docs/research/split-solver-grill-brief-2026-07-19.md sections 3, 4, 6 and 10 (steps 1-5 done; step 6 machinery + 60t shakedown done 2026-07-20, see the step 6 checklist entry and docs/research/data/matrix-bakeoff60-1/analysis.md: native ALM lambda shipped, A18 sweep mode shipped as PKC_MATRIX_SWEEP, incumbent leads the 60t leg 6/6 vs 3/6 feasible). Finish the bake-off: (1) the user is producing 80/100-tick rungs (the corpus tick gap runs 66 to 176); when the captures land, add them as problems + band entries and run both contenders at 80/100t; (2) run the ALM tuning sweep at fixed lambda over the A18 grid (seeds/topk/cooking/gate), since the shakedown ran stage defaults only; (3) decide the warm-seed policy: matrix legs stay cold by the seedless rule, but the production persona race could legitimately hand ALM the incumbent's solution as a warm seed, which changes what the bake-off should measure; (4) the >64t incumbent question was run AS-IS per the standing recommendation (extending SLP with a travel term collides with the step-5 feasibility firewall); revisit only if the tuned-ALM 80/100t legs fail feasibility outright. Then declare the A6 verdict: winner takes the TASer persona, loser stays a graph node. After that, checklist step 7 (verifier verdict layer)."
