...
**Axis-locked momentum (45 cancellation)**:
The core of a neo/nix momentum setup. You strafe in the FIRST place for speed: forward-only acceleration is scaled by 0.98, strafing by 1.0, so a 45 strafe is 1.0/0.98 (~2%) faster every tick (see [[45 strafe]]). Doing that strafe at 45 degrees is what makes the boosted acceleration land on a world axis, which is what lets you sculpt clean 1-D momentum. Each tick, acceleration is added along the direction D (the raw key vector rotated by the facing): `accelX ~ strafe*cosF - forward*sinF`, `accelZ ~ forward*cosF + strafe*sinF`. At a fixed facing of 45 degrees, W+A gives `accelX = 0, accelZ > 0` (pure +Z) and S+D gives `accelX = 0, accelZ < 0` (pure -Z): the diagonal strafe cancels the diagonal facing so the full-magnitude (2%-boosted) acceleration lands exactly on one world axis, and W+A vs S+D are exact opposites along it. This buys (1) an axis lock (the perpendicular axis gets zero acceleration, so it stays pinned at 0, which a neo needs since it travels straight along the wall's axis), (2) bidirectional control at a single facing (build momentum with W+A, brake or reverse it with S+D, no camera turn), and (3) a byte-exact cancellation: brake until `|v*0.91| < 0.005` (the [[Momentum threshold]]) so the gate snaps the axis to exactly 0, then rebuild. The facing is held FIXED because any yaw wobble both leaks acceleration onto the pinned axis and shifts the integer sine-LUT bucket, desyncing the threshold crossing. So a momentum setup is not a facing search: it is a fixed facing plus a discrete W/S + A/D key pattern sculpting 1-D velocity. (A sprint-jump tick adds a 0.2 boost along the FACING, which is off-axis at 45 degrees, so cancellation setups are typically non-sprinting or axis-align the facing on the boosted tick.)
...
**Run ticks**:
Extra grounded W+sprint ticks inserted immediately before a jump to lengthen its run-up. Inside a jump solve range everything is airborne except the run-up, which is why the range's default slipperiness is AIR; a non-jump tick in the range carrying a ground (`DEFAULT`) slipperiness override therefore *is* a run tick. The run-ticks search owns them: it drops the ones already in the range, then tries every combination of run-tick counts across the jumps, solves each one, and keeps the combination with the best objective. Max ticks 0 therefore strips the range's run ticks. An `RT` constraint on a jump tick restricts how many run ticks that jump may receive; the angle solver itself ignores `RT`.
