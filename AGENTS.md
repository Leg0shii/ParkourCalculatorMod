...
- `docs/research/`: angle-solver design record, ILS global-solve notes, the cold-search + Strat Finder record (`stratfinder-coldsearch.md`)
...
| Strat finder (variant sweep) | `core/.../anglesolver/stratfinder/`: warm engine `StratVariants.java` + `StratPlans.java` + `StratFinder.java` (substitution search on a recording, smoothed line + measurement); world-anchored cold path `StratProblem.java` (start/landing/collision areas + abstract segments) -> `ProblemCompiler.java` (per-schedule synthesized solver-only saves; `JumpArcs.java` Y-arc durations) -> `BlockStratFinder.java` (per-segment `ColdStratFinder`/beam runs, collision gate, `ColdSnapshots.java` result-to-save); shared ranking `StratMeasure.java`/`StratMeasurements.java`/`StratDifficulty.java` (combinedV4 + per-edge shift leniency; metriclab `MeasurementEngine` delegates here), `CorpusIndex.java` + `resources/strats/library.json` (corpus provenance), `StratLabels.java`; controller `core/.../ColdStratController.java` (warm + cold merged job); UI `core/.../ui/coldfinder/ColdStratWidget.java` (definition panel + ranked table + leniency + 3D view) |
