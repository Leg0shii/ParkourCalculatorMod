...
| Strat finder (variant sweep) | `core/.../anglesolver/stratfinder/`: `StratVariants.java` (key-timing enumeration + timing families + row-derived samples), `StratPlans.java` (fmm/pessi/run+jam/mark/bwmm key-pattern grammar, shared with the parked test-side generation), `StratFinder.java` (per-variant scratch solves, ranking); controller `core/.../StratFinderController.java`; UI `core/.../ui/anglesolver/StratFinderWidget.java` |
